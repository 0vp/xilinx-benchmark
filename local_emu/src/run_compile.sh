...
source /opt/Xilinx/Vitis/2021.2/settings64.sh > /dev/null
source /opt/xilinx/xrt/setup.sh > /dev/null

PLATFORM=$AWS_PLATFORM
PLATFORM_NAME=$(echo "${AWS_PLATFORM##*/}" | sed 's/\.xpfm$//')
EMULATION_MODE="hw_emu"
SCRIPT_START_TIME=$(date +"%Y%m%d_%H%M%S")
MESSAGE=""

declare -a PROJECTS=("xrt_cpu_to_gmem" "ocl_cpu_to_gmem" "ocl_cpu_to_gmem_rw" "xrt_cpu_to_gmem_rw" "ocl_fpga_to_gmem")
declare -a EXECUTABLES=("hello_world_xrt -x" "hello_world" "hello_world" "hello_world_xrt -x" "hello_world")
declare -a XCLBINS=("vadd.xclbin" "vadd.xclbin" "vadd.xclbin" "vadd.xclbin" "vadd.xclbin")
...
async_project() {
    ...
    mkdir $PROJECT
    mkdir -p ../logs
    LOG_FILE="../logs/emu_"$PROJECT"_"$SCRIPT_TIME".log"
...
if [ "$EMULATION_MODE" = "hw_emu" ]; then
    for i in $(seq 0 3); do
        ...
        # Run each example
        LOG_FILE="../logs/emu_"$PROJECT"_"$SCRIPT_START_TIME".log"
        ...
        MESSAGE="Parsing emulation data..."; log_message
        python parse_summary.py $PROJECT
    done
fi
