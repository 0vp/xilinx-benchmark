...
// int DATA_SIZE = (DATA_SIZE * 1024 * 1024) / sizeof(uint32_t); // 256MB = 256 x 1024 x 1024 / 4Bytes (uint32_t)
...
int main(int argc, char* argv[]) {
    ...
    for (int DATA_SIZE : DATA_SIZES) {
        ...
        if (DATA_SIZE != DATA_SIZES[0]) {
            // while ./timeline_kernels..csv does not exist, wait for it
            while (std::ifstream("./timeline_kernels.csv").good() == false) {
                std::cout << "Waiting for timeline_kernels.csv to be generated..." << std::endl;
                sleep(1);
            }
            ...
            // get the previous data size
            int prev_data_size = DATA_SIZES[std::distance(DATA_SIZES.begin(), std::find(DATA_SIZES.begin(), DATA_SIZES.end(), DATA_SIZE)) - 1];
            std::string command = "cd ..; python3 ./parse_timeline_kernels.py " + std::to_string(prev_data_size) + " ocl_fpga_to_host";
            std::system(command.c_str());
        }

        // if last iteration
        // last iteration is when DATA_SIZE == 0, so we can have 1 more iteration.
        if (DATA_SIZE == 0) {
            return EXIT_SUCCESS;
        }
        ...
        // if (xcl::is_emulation()) {
        //     DATA_SIZE = 1024 / sizeof(uint32_t); // 1MB = 1024 / 4Bytes (uint32_t)
        // }
